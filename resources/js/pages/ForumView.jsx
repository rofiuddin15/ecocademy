...
const ForumView = () => {
    ...
    const fetchThreads = async () => {
        try {
            ...
            if (response.data.length > 0 && !activeThread && user && user.role !== 'instructor') {
                // Load details of the first thread by default (only for non-instructors)
    ...
    return (
        <>
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8 border-b border-outline-variant/20 pb-6">
                ...
                <div className="flex flex-col md:flex-row items-center gap-4 w-full md:w-auto">
                    ...
                    <button
                        ...
                        className="w-full md:w-auto bg-secondary text-on-secondary hover:bg-secondary/90 px-6 py-2.5 rounded-lg text-label-sm font-bold flex items-center justify-center gap-2 hover:scale-95 transition-all shadow-sm shrink-0 cursor-pointer"
                    >
            ...
            {isInstructor ? (
                /* Instructor Flat Layout */
                <div className="w-full">
                    {isCreatingThread ? (
                        /* Create New Thread Form (Full Width) */
                        <div className="bg-white p-6 rounded-lg border border-outline-variant/30 shadow-sm space-y-6 max-w-3xl mx-auto">
                            <div className="flex items-center justify-between border-b border-outline-variant/10 pb-4">
                                ...
                                <button 
                                    ...
                                    className="flex items-center gap-1 text-on-surface-variant hover:text-primary transition-colors text-label-sm font-bold cursor-pointer bg-transparent border-0"
                                >
                            ...
                            <form onSubmit={handleCreateThread} className="space-y-4">
                                ...
                                <div className="flex gap-2 justify-end">
                                    <button 
                                        ...
                                        className="bg-outline text-white px-6 py-2 rounded-lg text-label-sm font-bold cursor-pointer"
                                    >
                                        Batal
                                    </button>
                                    <button 
                                        ...
                                        className="bg-secondary text-on-secondary px-6 py-2 rounded-lg text-label-sm font-bold hover:scale-95 transition-all shadow-sm cursor-pointer"
                                    >
                        ...
                        /* Display Thread Details & Comments (Full Width) */
                        <div className="space-y-6 max-w-4xl mx-auto">
                            {/* Back Button */}
                            <button 
                                onClick={() => setActiveThread(null)}
                                className="inline-flex items-center gap-2 text-primary hover:text-primary/80 transition-colors text-label-sm font-bold cursor-pointer bg-transparent border-0"
                            >
                                <span className="material-symbols-outlined text-[18px]">arrow_back</span>
                                <span>Kembali ke Daftar Umpan Balik</span>
                            </button>
                            ...
                            <div className="bg-white p-6 rounded-lg border border-outline-variant/30 shadow-sm space-y-6">
                                ...
                                <div className="space-y-4 max-h-[50vh] overflow-y-auto pr-2">
                                    {activeThread.comments?.length === 0 ? (
                                        <p className="text-body-md text-on-surface-variant/80 italic text-center py-4">Belum ada tanggapan. Berikan tanggapan instruktur pertama Anda!</p>
                                    ) : (
                                        activeThread.comments?.map((comment) => (
                                            <div key={comment.id} className="p-4 bg-surface-container-low/20 rounded-lg border border-outline-variant/15 space-y-2">
                                                <div className="flex justify-between items-center text-[11px] text-on-surface-variant font-bold">
                                                    <span className={comment.user?.role === 'instructor' ? 'text-primary font-bold' : ''}>
                                                        {comment.user?.name || 'Anonim'} {comment.user?.role === 'instructor' && '(Instruktur)'}
                                                    </span>
                                ...
                                <form onSubmit={handleCreateComment} className="border-t border-outline-variant/10 pt-4 space-y-3">
                                    <textarea
                                        ...
                                        placeholder="Tulis tanggapan / arahan evaluasi Anda..."
                        ...
                        /* Flat List of Threads (Full Width) */
                        <div className="space-y-4">
                            <div className="flex items-center justify-between px-1">
                                <h3 className="text-label-sm font-bold text-primary">Daftar Umpan Balik & Diskusi</h3>
                                <span className="text-xs text-on-surface-variant font-medium">{filteredThreads.length} topik aktif</span>
                            </div>
                            ...
                            {isLoading && threads.length === 0 ? (
                                ...
                                <div className="p-12 text-center bg-white rounded-lg border border-outline-variant/30 shadow-sm">
                                    <span className="material-symbols-outlined text-[48px] text-on-surface-variant/30 mb-2">forum</span>
                                    <p className="text-label-sm text-on-surface-variant">Diskusi tidak ditemukan.</p>
                                </div>
                            ) : (
                                <div className="grid grid-cols-1 gap-3">
                                    {filteredThreads.map((thread) => (
                                        <div
                                            ...
                                            className="w-full p-5 rounded-xl border border-outline-variant/35 bg-white shadow-sm hover:shadow-md hover:border-primary transition-all flex flex-col justify-between gap-4 cursor-pointer"
                                        >
                                            <div className="flex justify-between items-start gap-4">
                                                <div className="space-y-1">
                                                    <h4 className="text-headline-sm font-bold text-primary hover:underline">
                                                        {thread.title}
                                                    </h4>
                                                ...
                                                <button
                                                    className="bg-primary-fixed/10 hover:bg-primary-fixed/20 text-primary text-xs font-bold px-4 py-2 rounded-lg transition-colors shrink-0 border-0 pointer-events-none"
                                                >
                                                    Buka Diskusi
                                                </button>
                                            </div>

                                            <div className="flex flex-wrap items-center justify-between gap-2 text-xs text-on-surface-variant pt-3 border-t border-outline-variant/10 font-medium">
                                                <div className="flex items-center gap-4">
                                                    <span className="flex items-center gap-1.5">
                                                        <span className="material-symbols-outlined text-[16px] text-primary">person</span>
                                                        Oleh: <strong className="text-on-surface">{thread.user?.name || 'Anonim'}</strong>
                                                    </span>
                                                    <span className="flex items-center gap-1.5">
                                                ...
                                                <span className="flex items-center gap-1 bg-secondary-container/20 text-secondary-fixed-variant px-3 py-1 rounded-full text-[11px] font-bold">
                                                    <span className="material-symbols-outlined text-[14px]">forum</span>
                                                    {thread.comments_count || 0} Tanggapan
                                                </span>
                                            </div>
                                        </div>
                                    ))}
                ...
                /* Original Student Layout (Split Pane) */
